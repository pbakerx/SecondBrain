# 🧠 Philip Baker — Second Brain

_Your command center. Everything you're building, in one place. Updated continuously._

**Last updated:** 2026-07-15

---

## 🔥 Today's Focus

> The 1–3 things that actually move the needle today. Reviewed every morning at 7 AM.

1. **Brock Institute** — confirm dev site accuracy, wire contact forms, make client changes → [open](Projects/brock-institute.md)
2. _(add)_
3. _(add)_

See the running log → [Daily Log](daily-log.md)

---

## 🚀 Active Projects
_Time-bound, has a finish line._

| Project | Status | Next action | Link |
|---|---|---|---|
| Brock Institute site | 🟡 In progress | Confirm dev site + wire forms | [→](Projects/brock-institute.md) |
| AechTech Voice Agents | 🟢 Pipeline proven | Scale outreach | [→](Projects/aechtech-voice-agents.md) |

---

## 🌱 Areas (Businesses)
_Ongoing, no end date. Your three pillars._

| Area | One-liner | Link |
|---|---|---|
| AI Implementation | Help businesses put AI into their processes | [→](Areas/ai-implementation.md) |
| AI Creative | Portfolio-grade creative made with AI | [→](Areas/ai-creative.md) |
| App Development (AechTech LLC) | Building Assist.ai + client apps | [→](Areas/app-development.md) |

---

## 📚 Resources
_Reference material, swipe files, taglines, research._

- [Taglines & Juicy Lines](Resources/taglines.md)
- [Second Brain — how others do it (X scan)](Resources/second-brain-methods.md)

---

## 🗄️ Archive
_Done or paused. Out of sight, still searchable._ → [Archive/](Archive/)

---

## How this works
- **Projects** = things with a deadline. **Areas** = businesses you run forever. **Resources** = reference. **Archive** = closed.
- **Capture fast:** dump anything into the top of [Daily Log](daily-log.md); I sort it into the right place.
- I review this file with you every morning at **7 AM** and push you toward the next action.
